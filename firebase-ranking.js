...
function profileKey(v){
 return name(v).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_+|_+$/g,'').slice(0,40)||'jogador';
}
async function saveProfile(data){
 const user=await ensureAuth();
 const key=profileKey(data.name);
 const ref=db.collection('mathQuestProfiles').doc(key);
 const payload={
  name:name(data.name),grade:grade(data.grade),points:Number(data.points)||0,level:Number(data.level)||1,
  rank:String(data.rank||'Bronze'),coins:Math.max(0,Number(data.coins)||0),
  character:character(data.character),
  customization:data.customization||{},
  ownedSkins:Array.isArray(data.ownedSkins)?data.ownedSkins:['default'],
  equippedSkin:String(data.equippedSkin||'default'),
  updatedAt:firebase.firestore.FieldValue.serverTimestamp(),
  authUid:lastAuthUid(user)
 };
 await ref.set(payload,{merge:true});
 return payload;
}
function lastAuthUid(user){return user&&user.uid?user.uid:'';}
async function loadProfile(nick){
 await init({requireAuth:true});
 const ref=db.collection('mathQuestProfiles').doc(profileKey(nick));
...
async function saveScore(data){
 ...
 const ref=db.collection('mathQuestRanking').doc(g).collection('players').doc(profileKey(data.name));
 let saved=null;
 await db.runTransaction(async tx=>{
  ...
  const incomingTotal=Number(data.totalPoints);
  const points=Number.isFinite(incomingTotal)
    ? Math.max(Number(old.points)||0,incomingTotal)
    : (Number(old.points)||0)+Math.max(0,Math.floor(Number(data.score)||0));
...
async function loadScore(g,nick){
 await init({requireAuth:true});
 const ref=db.collection('mathQuestRanking').doc(grade(g)).collection('players').doc(profileKey(nick||''));
 const snap=await ref.get();
 return snap.exists?{id:snap.id,...snap.data()}:null;
}
...
window.FirebaseRanking={isConfigured,init,ensureAuth,saveScore,loadScore,saveProfile,loadProfile,getAllPlayers,watchRanking,getRankInfo,explainError:errorText,RANKS};
})();