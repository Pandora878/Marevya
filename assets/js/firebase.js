...
(function () {
  ...
  window.MarevyaFirebaseReady = (async function () {
    if (!window.firebase) {
    ...
    // Analytics é opcional para o funcionamento do login.
    // Se o SDK de Analytics estiver disponível, inicializamos sem impedir o Auth.
    let analytics = null;
    try {
    ...
    window.marevyaDB = db;
    window.marevyaAnalytics = analytics;

    return { app: firebase.app(), auth, db, analytics };
  })();
})();
